...
echo ""
psql "$DB_URL" -c "SELECT id, name, price, currency FROM tickets ORDER BY id;"

echo ""
echo "2. currency が jpy でないチケットを確認中..."
echo ""
psql "$DB_URL" -c "SELECT id, name, currency FROM tickets WHERE currency IS NULL OR currency != 'jpy';"
